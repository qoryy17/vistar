        ...
        <div class="container">
            ...
            <div itemscope itemtype="https://schema.org/Product">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="d-flex flex-row gap-2">
                            ...
                            <a title="Bagikan {{ $product->produk }} ke Whatsapp"
                                href="https://api.whatsapp.com/send/?text={{ urlencode('Lihat ' . $product->produk . ' Workshop dari ' . config('app.name') . ' disini ' . url()->current()) }}&type=custom_url&app_absent=0"
                                target="_blank" class="share-it share-wa">
                                <i class="mdi mdi-whatsapp"></i>
                                <span class="fw-bold">Whatsapp</span>
                            </a>
                            <a title="Bagikan {{ $product->produk }} ke Twitter/X"
                                href="https://x.com/intent/tweet?url={{ url()->current() }}&text={{ urlencode('Lihat ' . $product->produk . ' Workshop dari ' . config('app.name')) }}"
                        ...
                        @if (@$product->category->status === 'Berbayar')
                            <div class="mt-4">
                                @if ($order)
                                    <a href="{{ route('customer.get-sertikom', ['category' => 'workshop']) }}"
                                        title="{{ $product->produk }}"
                                        class="btn btn-pills btn-soft-primary d-block d-md-inline-block">
                                        Lihat Workshop <i class="mdi mdi-arrow-right"></i>
                                    </a>
                                @else
                                    @if (date('Y-m-d') > $product->tanggal_mulai)
                                        <div class="alert bg-soft-warning fw-medium" role="alert">
                                            ...
                                            Workshop ini sedang berlangsung tidak dapat dipesan !
                                        </div>
                                    @else
                                        <form
                                            action="{{ route('mainweb.cart-sertikom-add', ['idProdukSertikom' => Crypt::encrypt($product->id), 'category' => 'workshop']) }}"
                                            method="POST">
                                            @csrf
                                            @method('POST')
                                            <button type="submit"
                ...
                <div class="mt-4 mb-2 border-top">
                    <h5 class="mt-3">Informasi Workshop</h5>
                    <p style="text-align: justify;">
                        {!! $product->deskripsi !!}
                    </p>

                    <h5 class="mt-3">Benefit Workshop</h5>
                    @php
                    ...
                    <h5 class="mt-3">Jadwal Workshop</h5>
    ...
    @php
        ...
        $breadcrumbItemList = [
            ...
            [
                ...
                'item' => route('mainweb.product-sertikom', ['category' => 'Workshop']),
            ],
            [
                '@type' => 'ListItem',
                'position' => ++$breadcrumbItemListPosition,
