        ...
        <div class="container">
            ...
            <div itemscope itemtype="https://schema.org/Product">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="d-flex flex-row gap-2">
                            ...
                            <a title="Bagikan {{ $product->produk }} ke Whatsapp"
                                href="https://api.whatsapp.com/send/?text={{ urlencode('Lihat ' . $product->produk . ' Pelatihan dari ' . config('app.name') . ' disini ' . url()->current()) }}&type=custom_url&app_absent=0"
                                target="_blank" class="share-it share-wa">
                                <i class="mdi mdi-whatsapp"></i>
                                <span class="fw-bold">Whatsapp</span>
                            </a>
                            <a title="Bagikan {{ $product->produk }} ke Twitter/X"
                                href="https://x.com/intent/tweet?url={{ url()->current() }}&text={{ urlencode('Lihat ' . $product->produk . ' Pelatihan dari ' . config('app.name')) }}"
                                ...
                                @else
                                    @if (date('Y-m-d') > $product->tanggal_mulai)
                                        <div class="alert bg-soft-warning fw-medium" role="alert">
                                            ...
                                            Pelatihan ini sedang berlangsung tidak dapat dipesan !
                                        </div>
                                    @else
                                        <form
                                            action="{{ route('mainweb.cart-sertikom-add', ['idProdukSertikom' => Crypt::encrypt($product->id), 'category' => 'pelatihan']) }}"
                                            method="POST">
                                            @csrf
                                            @method('POST')
                ...
                <div class="mt-4 mb-2 border-top">
                    <h5 class="mt-3">Informasi Pelatihan</h5>
                    <p style="text-align: justify;">
                        {!! $product->deskripsi !!}
                    </p>

                    <h5 class="mt-3">Instruktur Pelatihan</h5>
                    <p style="text-align: justify;">
                        {{ $product->instructor->instruktur }}
                        <br>
                        Keahlian : {{ $product->instructor->keahlian }}
                        <br>
                        {{ $product->instructor->deskripsi }}
                    </p>

                    <h5 class="mt-3">Benefit Pelatihan</h5>
                    @php
                        $benefit = App\Helpers\BerandaUI::benefitTraining();
                    @endphp
                    ...
                    <h5 class="mt-3">Jadwal Pelatihan</h5>
            ...
            @if ($recommendProducts->isNotEmpty())
                <div class="row mt-6 border-top">
                    <div class="col-lg-12 col-md-12 mt-4 pt-2">
                        <h2 class="fs-5">Rekomendasi Produk Pelatihan Pilihan</h2>
                        <p class="text-muted" style="text-align: justify;">
                            Temukan pilihan pelatihan terbaik untuk meningkatkan keterampilan Kamu! Dari topik dasar hingga
                            lanjutan, setiap kursus dirancang untuk membantu Kamu mencapai tujuan karier dengan lebih cepat
                            dan percaya diri. Pilih pelatihan yang sesuai dan mulai tingkatkan potensi Kamu hari ini!
                        </p>
                    </div>
                            ...
                            <div class="card blog blog-primary rounded border-0 shadow overflow-hidden" itemscope
                                ...
                                <div class="card-body content">
                                    ...
                                    <a itemprop="url" href="{{ $url }}"
                                        title="Lihat Pelatihan {{ $row->produk }}" class="title text-dark h5">
                ...
                <div class="row mt-5">
                    <div class="col-lg-12 text-center">
                        <a title="Lihat Semua Pelatihan {{ config('app.name') }}"
                            href="{{ route('mainweb.product-sertikom', ['category' => 'pelatihan']) }}"
                            class="btn btn-pills btn-soft-primary">
                            Lihat Semua Pelatihan <i class="uil uil-arrow-right"></i>
                        </a>
                    </div>
                </div>
            @endif
    ...
    @php
        ...
        $breadcrumbItemList = [
            ...
            [
                ...
                'item' => route('mainweb.product-sertikom', ['category' => 'pelatihan']),
            ],
            [
                '@type' => 'ListItem',
                'position' => ++$breadcrumbItemListPosition,
